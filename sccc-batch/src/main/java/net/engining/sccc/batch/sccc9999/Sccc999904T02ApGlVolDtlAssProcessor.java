...
@Service
@StepScope
public class Sccc999904T02ApGlVolDtlAssProcessor implements ItemProcessor<List<String>, RepeatStatus> {

	private static final Logger log = LoggerFactory.getLogger(Sccc999904T02ApGlVolDtlAssProcessor.class);
	...
	public RepeatStatus process(List<String> item) throws Exception {
		if (item != null) {
			...
			bean.setKeys(item);
			bean.setTxnDetailType(TxnDetailType.T);
			log.debug("将为Step9999ApGlVolDtlAss发送消息到MQ，keys={}", JSON.toJSONString(item));
