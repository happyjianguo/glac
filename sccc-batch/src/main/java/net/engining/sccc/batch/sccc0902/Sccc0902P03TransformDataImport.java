...
@StepScope
@Validated
public class Sccc0902P03TransformDataImport implements ItemProcessor<EveryDayAccountingBean, BtEodTxnImport> {
	...
	public BtEodTxnImport process(EveryDayAccountingBean item) throws Exception {
		...
		// 业务数据检查
		bizValidatorService.validateTrans(item);
		...
		btEodTxnImport.setTxnType(TxnTypeDef.TRANSFO);
