...
@StepScope
@Validated
public class Sccc0902P01InteAccrualDataImport implements ItemProcessor<EveryDayAccountingBean, BtEodTxnImport> {
	...
	public BtEodTxnImport process(EveryDayAccountingBean item) throws Exception {
		...
		if (btEodTxnImport != null) {
			throw new ErrorMessageException(ErrorCode.CheckError, String.format("数据重复，请核对文件！错误的流水号 %s", item.getTxnSerialNo()));
		}
		//业务数据检查
		bizValidatorService.validateInte(item);
		...
		btEodTxnImport.setTxnType(TxnTypeDef.INTEACC);
