...
@Service
@StepScope
public class Sccc999902T02ApGlTxnProcessor implements ItemProcessor<List<String>, RepeatStatus> {

	private static final Logger log = LoggerFactory.getLogger(Sccc999902T02ApGlTxnProcessor.class);
	...
	public RepeatStatus process(List<String> item) throws Exception {
		if(item !=null){
			...
			bean.setKeys(item);
			bean.setTxnDetailType(TxnDetailType.C);
			batchStepMQProducer.senderMsg4Step9999(bean);
			log.debug("将为Step9999ApGlTxn发送消息到MQ，keys={}", JSON.toJSONString(item));
		}
